...
Http::FilterHeadersStatus ResponseMapFilter::encodeHeaders(Http::ResponseHeaderMap& headers,
  ...
  do_rewrite_ =
      config_->response_map()->match(request_headers_, headers, encoder_callbacks_->streamInfo());
...
void ResponseMapFilter::doRewrite(void) {
  ...
  config_->response_map()->rewrite(request_headers_, *response_headers_,
                                   encoder_callbacks_->streamInfo(), new_body, new_content_type);
